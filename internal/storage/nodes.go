...
func (r *TrafficRepository) ListNodes(ctx context.Context, username string) ([]Node, error) {
	...
	rows, err := r.db.QueryContext(ctx, `SELECT id, username, raw_url, node_name, protocol, parsed_config, clash_config, enabled, COALESCE(tag, 'personal'), COALESCE(original_server, ''), COALESCE(probe_server, ''), COALESCE(tags, '[]'), chain_proxy_node_id, COALESCE(relay_group_name,''), COALESCE(relay_group_node_ids,'[]'), COALESCE(probe_enabled, 0), singbox_server_id, created_at, updated_at FROM nodes WHERE username = ? ORDER BY created_at DESC`, username)
	if err != nil {
...
func (r *TrafficRepository) GetNode(ctx context.Context, id int64, username string) (Node, error) {
	...
	var probeEnabled int
	var tagsJSON, relayGroupNodeIDsJSON string
	row := r.db.QueryRowContext(ctx, `SELECT id, username, raw_url, node_name, protocol, parsed_config, clash_config, enabled, COALESCE(tag, 'personal'), COALESCE(original_server, ''), COALESCE(probe_server, ''), COALESCE(tags, '[]'), chain_proxy_node_id, COALESCE(relay_group_name,''), COALESCE(relay_group_node_ids,'[]'), COALESCE(probe_enabled, 0), singbox_server_id, created_at, updated_at FROM nodes WHERE id = ? AND username = ? LIMIT 1`, id, username)
